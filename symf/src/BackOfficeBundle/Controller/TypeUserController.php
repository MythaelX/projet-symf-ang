...
class TypeUserController extends Controller
{
    ...
    public function indexAction()
    {
        ...
        $typeUsers = $em->getRepository('BackOfficeBundle:TypeUser')->findAll();
    ...
    public function newAction(Request $request)
    {
        ...
        $form = $this->createForm('BackOfficeBundle\Form\TypeUserType', $typeUser);
    ...
    {
        $deleteForm = $this->createDeleteForm($typeUser);
        $editForm = $this->createForm('BackOfficeBundle\Form\TypeUserType', $typeUser);
        $editForm->handleRequest($request);

        if ($editForm->isSubmitted() && $editForm->isValid()) {
            $this->getDoctrine()->getManager()->flush();

            return $this->redirectToRoute('typeuser_edit', array('id' => $typeUser->getId()));
        }
